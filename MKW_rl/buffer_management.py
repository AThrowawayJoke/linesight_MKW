...
def get_potential(state_float):
    ...
    ) # + (config_copy.shaped_reward_point_to_vcp_ahead * (vector_vcp_to_vcp_further_ahead_normalized[2] - 1)) # TODO: Fix vector shape to match previous logic
...
def fill_buffer_from_rollout_with_n_steps_rule(
    ...
    engineered_airtime_reward=0,
    engineered_external_velocity_reward=0.0,
    engineered_close_to_vcp_reward=0,
    engineered_trick_reward=0,
):
    ...
    for i in range(1, n_frames): # run for each frame of the rollout
        ...
        if type(rollout_results["state_float"][i]) != float and rollout_results["state_float"][i]["race_data"]["state"] == 2: # Apply these rewards during the race (Not race finished, not during countdown timer)
            ...
            # ILLEGAL ZONE ENFORCEMENT
            #if rollout_results["state_float"][i]["kart_data"]["position"][2] < config_copy.DC_punish_line_z:
        ...
        if i < n_frames - 1: # apply these rewards unless this is the finish frame
            ...
            # Only apply these rewards during the actual race (Not race finished, not during countdown timer)
            if type(rollout_results["state_float"][i]) != float and rollout_results["state_float"][i]["race_data"]["state"] == 2:
                ...
                # VCPs preferred to avoid inconsistent race_completion values on certain tracks
                # temp_completion_reward = (
                ...
                """if engineered_close_to_vcp_reward != 0:
                    reward_into[i] += np.interp(max(config_copy.engineered_reward_min_dist_to_cur_vcp,
                        ...
                        [config_copy.engineered_reward_min_dist_to_cur_vcp, config_copy.engineered_reward_max_dist_to_cur_vcp],
                        [0, -2] # normalizing to 1, -1 using np.interp so when we multiply by engineered reward we are reasonable
                    ) * (1 / config_copy.temporal_mini_race_duration_actions) * engineered_close_to_vcp_reward * 0

                if rollout_results["state_float"][i]["airtime"] == 0: # punish being on the ground
                    reward_into[i] += (engineered_airtime_reward / config_copy.temporal_mini_race_duration_actions) * -1
                ...
                external_velocity = math.sqrt(rollout_results["state_float"][i]["kart_data"]["external_velocity"][0]**2 + 
                                              rollout_results["state_float"][i]["kart_data"]["external_velocity"][2]**2)
                previous_external_velocity = math.sqrt(rollout_results["state_float"][i - 1]["kart_data"]["external_velocity"][0]**2 + 
                                                       rollout_results["state_float"][i - 1]["kart_data"]["external_velocity"][2]**2)
                
                # punish EV below expected value
                external_velocity = external_velocity - config_copy.expected_ev_punish_line
                previous_external_velocity = previous_external_velocity - config_copy.expected_ev_punish_line

                external_velocity_reward = external_velocity * engineered_external_velocity_reward # reward for EV
                shaped_ev_reward = (external_velocity - previous_external_velocity) * engineered_external_velocity_reward * config_copy.shaped_ev_reward_multiplier # shaped reward for EV (total over race = 0)
